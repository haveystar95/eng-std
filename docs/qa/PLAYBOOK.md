...
**На экране.** Тот же список; заголовком карточки должно быть слово **изучаемого языка коллекции**
(в этом прогоне пара `ru→en`, значит английское, а не русское). Мгновенная строка тоже должна
отдать эквивалент на изучаемом языке.
...
- **Панель симулятора в Claude Code может быть выключена флагом раскатки — целиком.** Тогда не
  работает и ввод: `attach`/`tap` отвечают «has stopped retrying after repeated crashes», и это не
  чинится повтором. Прогон в таком режиме ведётся руками: тапает и свайпает ЮЗЕР, а сессия снимает
  кадры через `xcrun simctl io booted screenshot <путь>` — он от панели не зависит и работает всегда
  (`xcrun simctl launch booted com.denis.engstd` тоже). Отличие от строки выше в том, что там падал
  только `screenshot`, а здесь недоступен весь канал ввода. Наблюдалось на A-4 (2026-08-24).
  Это ограничение СТЕНДА, а не приложения, и в отчёт оно пишется прямо — как и всякий прогон,
  часть которого прошла без экрана.
