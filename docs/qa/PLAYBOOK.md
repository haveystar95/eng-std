...
| QA-BUG-3 | 2026-08-22 | смоук 1.2 / 1.3 | **Полоса «Дневной цели» не показывает заливку.** Замерено по пикселям на обоих экранах: при 3/20 (15%) — сплошной rgb(210,206,199), при 8/20 (40%) — сплошной rgb(215,212,207), перехода на отметке нет. Полоса декоративная. | открыт, не чинился |
...
| QA-OBS-6 | 2026-08-22 | сценарий 3, `speaking` | Пояснения к системным разрешениям (`NSMicrophoneUsageDescription`, `NSSpeechRecognitionUsageDescription`) зашиты в `ios/Runner/Info.plist` **по-русски**, локализованного `InfoPlist.strings` нет. При английском интерфейсе диалог показывает русский текст. | косметика / релизный риск |
| QA-OBS-7 | 2026-08-22 | сценарий 3, `speaking` | **Надпись «можно пропустить» появляется раньше кнопки «Пропустить».** Сообщение показывается с ПЕРВОГО отказа микрофона, а кнопка — только на `_attempts >= SpokenAnswer.maxChannelAttempts` (3). При навсегда отклонённом разрешении надо трижды нажать микрофон впустую; всё это время единственное доступное действие — «Не помню», а это лапс. Проверено на симуляторе. | открыт, не чинился |
