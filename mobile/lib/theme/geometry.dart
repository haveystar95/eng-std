...
abstract final class AppRadii {
  ...
  static const sheet = 16.0; // поднятый лист ВНУТРИ статьи (значение, пример, лестница) — «Фаза 3»
...
/// Карточка слова — направление «Фото-герой» (макет «Слова · фаза 3», кадры 06/07/09).
abstract final class AppWordCard {
  /// Фото во всю ширину. 246 — главный экран; 230 — просмотр из папки (кадр 09) и 180, когда
  /// шапке нечего показывать, кроме кнопки «назад» (кадр 07).
  static const heroHeight = 246.0;
  static const heroHeightFromFolder = 230.0;

  /// Слова без фотографии. Блок ниже и нейтральный: композиция та же, дыры нет — и никто не ждёт
  /// картинку, которой не будет.
  static const heroHeightPlate = 132.0;

  /// Лист бумаги заезжает на фото — термин садится ровно на стык.
  static const heroOverlap = 22.0;

  /// Круглая кнопка над фото (назад / меню) и круглый динамик у термина.
  static const overlayButton = 40.0;
  static const speakButton = 46.0;

  /// Врезка-фото в результате поиска (кадр 03) — узнать, а не рассматривать.
  static const inlinePhoto = 88.0;

  /// Пара «главное действие + выбор папки» внизу карточки.
