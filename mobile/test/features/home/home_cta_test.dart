...
void main() {
  group('computeHomeCta — due → learn/limit → triage → none', () {
    test(
      'due terms win, whatever else exists — and are never gated by the new quota (F13 rule 1)',
      () {
    ...
    test(
      'quota spent but learnable words remain → limitReached, not a blocked session (F13 rule 3)',
      () {
        final cta = computeHomeCta(
          due: 0,
          learnable: 5,
    ...
    test(
      'no learnable words + quota 0 → triage, NOT limitReached (limit is about pending new words)',
      () {
        final cta = computeHomeCta(
          due: 0,
          learnable: 0,
    ...
    test(
      'nothing eligible → none (F13 rule 4: all learned, due 0, no new — screen shows all-done)',
      () {
        final cta = computeHomeCta(
          due: 0,
          learnable: 0,
