...
void main() {
  testWidgets('«Не знаю» (unknown) is the only filled verdict — rule 20', (tester) async {
    await tester.pumpWidget(
      _host(const VerdictButton(kind: VerdictKind.unknown, label: 'Не знаю')),
    );
  ...
  testWidgets('«Не уверен» (unsure) has no fill', (tester) async {
    await tester.pumpWidget(
      _host(const VerdictButton(kind: VerdictKind.unsure, label: 'Не уверен')),
    );
    expect(_fillOf(tester).color, Colors.transparent);
  });

  testWidgets('«Знаю» (known) has no fill', (tester) async {
    await tester.pumpWidget(_host(const VerdictButton(kind: VerdictKind.known, label: 'Знаю')));
    expect(_fillOf(tester).color, Colors.transparent);
  });
  ...
  testWidgets('tap fires onPressed', (tester) async {
    var tapped = false;
    await tester.pumpWidget(
      _host(VerdictButton(kind: VerdictKind.known, label: 'Знаю', onPressed: () => tapped = true)),
    );
