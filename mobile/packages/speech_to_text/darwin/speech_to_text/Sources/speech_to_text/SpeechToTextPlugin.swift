...
public class SpeechToTextPlugin: NSObject, FlutterPlugin {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case SwiftSpeechToTextMethods.listen.rawValue:
      ...
      // FORK-LOCAL ADDITION, see UPSTREAM.md — QA-20's contextualStrings passthrough. Absent (old
      // callers, and every call this fork doesn't opt into) means an empty list, i.e. no request
      // vocabulary hint, which is exactly upstream 7.4.0's behaviour.
      ...
      guard let listenMode = ListenMode(rawValue: listenModeIndex) else {
        ...
        if #available(iOS 13.0, *) {
            ...
            let capturedContextualStrings = contextualStrings
            Task {
                listenForSpeech(
                    ...
                    enableHaptics: capturedEnableHaptics, contextualStrings: capturedContextualStrings)
            }
        } else {
            listenForSpeech(
  ...
  private func listenForSpeech(
    ...
    do {
      ...
      // FORK-LOCAL ADDITION, see UPSTREAM.md — QA-20's contextualStrings passthrough: tells the
      // recogniser the vocabulary it should expect, the difference between transcribing a close
      // mishearing and the word actually meant.
