...
class SpeechToText {
  ...
  ///
  /// [contextualStrings] — FORK-LOCAL ADDITION, see UPSTREAM.md. Words the caller expects to
  /// hear, passed straight through to `SFSpeechRecognitionRequest.contextualStrings` on iOS.
  /// Null or empty (the default) is a complete no-op: [listen] takes exactly the code path
  /// upstream 7.4.0 does. Ignored on platforms other than iOS.
  Future listen(
    ...
    try {
      var started = await _startListening(usedOptions, contextualStrings);
      if (started) {
  ...
  /// FORK-LOCAL ADDITION, see UPSTREAM.md. The plugin's own method channel — the same one
  /// [SpeechToTextPlatform.instance] talks to on iOS/Android. Used directly, instead of through
  /// the interface, ONLY when [contextualStrings] is non-empty: [SpeechListenOptions] (in the
  /// separate, un-forked `speech_to_text_platform_interface` package) has no field for it.
  static const MethodChannel _rawChannel =
      MethodChannel('plugin.csdcorp.com/speech_to_text');

  /// Starts the platform listen session. With no [contextualStrings], this is byte-identical to
  /// upstream 7.4.0's inline call. With [contextualStrings], it builds the `listen` method call's
  /// argument map itself — mirroring `MethodChannelSpeechToText.listen()` in
  /// speech_to_text_platform_interface 2.4.0 field-for-field — and invokes the channel directly,
  /// adding one extra `contextualStrings` key. Keep this in sync with that method if the pinned
  /// platform_interface version ever changes (see UPSTREAM.md).
  Future<bool> _startListening(
      SpeechListenOptions options, List<String>? contextualStrings) async {
    if (contextualStrings == null || contextualStrings.isEmpty) {
      return await SpeechToTextPlatform.instance
          .listen(localeId: options.localeId, options: options);
    }
    final params = <String, dynamic>{
      'partialResults': options.partialResults,
      'onDevice': options.onDevice,
      'listenMode': options.listenMode.index,
      'sampleRate': options.sampleRate,
      'enableHaptics': options.enableHapticFeedback,
    ...
    if (options.localeId != null) {
      params['localeId'] = options.localeId;
    }
    return await _rawChannel.invokeMethod<bool>('listen', params) ?? false;
  }
