...
(`GenerationPipeline::assemble`), поэтому «EXACTLY {{size}}» в промпте и «размер приблизителен» в
валидаторе друг другу не противоречат — проверено, это не аномалия.

**Обновление (QA-OBS-9, коммит `61be58a`):** перезаказ больше не доезжает до пользователя. На
выходе `assemble` излишек срезается до запрошенного `size` — берутся первые N выживших в порядке
модели (`trimToRequested`), и коллекция, `delivered_count` и промпт-кеш несут один и тот же набор.
Перезаказ остался страховкой от недодачи и отсева, а не прибавкой к заказу; недодача по-прежнему
возможна и отгружается коротким прогоном, а не падает. Так что «размер приблизителен» теперь верно
только вниз: больше запрошенного коллекция не отдаёт.
