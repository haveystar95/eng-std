...
- [ ] **Сравнения МОДЕЛЕЙ пока нет** — участвовал один провайдер. У команды xAI нет кредитов
  (403 на каждом вызове), Anthropic-ключа нет, у OpenAI кредиты кончились посреди второго
  прогона. Как только появится второй живой ключ: `generation:bakeoff --pace=8000` и
  сравнение готово — машина построена и проверена.
