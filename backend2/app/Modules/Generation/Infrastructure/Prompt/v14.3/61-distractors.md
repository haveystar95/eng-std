...
All three are PLAIN TEXT. Do not wrap them in quotation marks, brackets or any other punctuation
that is not part of the fragment itself: the span is matched against your sentence character for
character, so an added `"` makes it unfindable and the whole candidate is discarded.

Worked example — given the example sentence `The post office is next to the museum.`, one candidate
is exactly this object (in English; the shape is the same in any language):
...
`error_span` is taken from that candidate's own `sentence`, not from the example; putting
`correction` where it sits reproduces the example word for word.
