## `description` — what it means, in {{target_lang}}, without naming it

The learner is shown this text ALONE and has to recognise the word from it. It is not a note beside
the card; it is the question one of the exercises asks. So:
...
- **It must NOT contain `text`, or any form of it.** «A place where you keep money» is a description
  of *bank*. «A bank is a place where you keep money» is not — it hands the answer over and the card
  asks nothing. This also rules out the obvious dodges: not the plural, not the verb form, not the
  same root inside a longer word. A description that names its own word is thrown away downstream, so
  the item ends up with none.
- **In {{target_lang}} only**, like `example`. Not a {{source_lang}} gloss and not a translation of
  `translation` — that field already exists one line up and this is a different product.
- Describe what it MEANS, not what part of speech it is. Not «an adjective used about weather».
- Where two senses are common, describe the one `example` uses, and only that one.

    TERM: reimbursement → "Money that a company pays back to you after you spend your own."
    TERM: windshield    → "The large glass at the front of a car that you look through when driving."
    TERM: jump-start    → "To start a car with a dead battery by using power from another car."
