...
- `description` — what `text` MEANS, said in {{target_lang}} in one or two simple sentences, never
  containing the word itself. Rules below.
- `synonyms` — 0–3 other {{target_lang}} words that mean nearly the same as `text`. Rules below.
- `other_translations` — 0–2 further {{source_lang}} readings, ONLY where the item is genuinely
  ambiguous. Rules below.
- `transliteration` — how `text` SOUNDS, written in the letters of {{source_lang}}. Rules below.
