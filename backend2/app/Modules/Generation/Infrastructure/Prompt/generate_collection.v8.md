...
### Keep every part of the term — pronouns above all
...
So the translation must keep, one for one:

- **the addressee and the speaker** — `us` → «нам/нас», `me` → «мне/меня», `you`/`your` →
  «вы/вас/вам/ваш», `we`/`our` → «мы/наш», `them`/`their` → «им/их»;
...
Worked through on the item that failed: `Tell us about a challenge you faced` was translated
«Расскажите о вызове, с которым вы столкнулись». It reads well and it is wrong as a question,
because «нам» is gone and `Tell me…`, `Tell us…` and `Describe…` all answer it equally. The
translation the learner needs is «Расскажите **нам** о вызове, с которым вы столкнулись» — clumsier
by a word, and unambiguous.

Same failure, same fix: `Tell us about your experience` is «Расскажите нам о **вашем** опыте», not
«Расскажите о своём опыте».

**Accuracy beats fluency here, every time.** If the smooth translation and the exact one differ,
ship the exact one. A slightly stiff question the learner can answer is worth more than an elegant
one that marks a correct answer wrong.
...
rest of the set in front of you. For each item, all six must hold:
...
4. **Nothing dropped.** Every pronoun and every qualifier in `text` has its counterpart in
   `translation`. Point at each of `us`/`me`/`you`/`your`/`we`/`our` in `text` in turn and find the
   word that carries it on the other side. If one has no counterpart, put it back, even if the
   sentence reads better without it.
5. **Target side.** `text` and `example` contain no {{source_lang}} at all.
6. **The example teaches.** `example` is not the same sentence as `text`. Put them side by side: if
   they read identically, rewrite the example so it shows the term in a situation.

An item that fails any of the six is FIXED, not shipped. If you cannot fix it, replace the item
